...
class Client
{
    ...
    private function getHttpClient(): HttpRequest
    {
        ...
        if (!$cfg->getKeepAlive())
        {
            return isset($this->httpClient) ? $this->httpClient : new HttpRequest();
        }
        ...
        if ($this->haveSwoole && Coroutine::getCid() > 0)
        {
            ...
            if (!isset($context[self::class][$key]))
            {
                $context[self::class][$key] = new HttpRequest();
            }

            return $context[self::class][$key];
        }
